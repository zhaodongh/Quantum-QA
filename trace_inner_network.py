...
if __name__ == '__main__':
	...
	Logistic_input = diag_feature.flatten(2)
	layer2 = LogisticRegression(Logistic_input, embedding_ndim, class_num)

	# layer3 = HiddenLayer(rng, Logistic_input, embedding_ndim + 1, embedding_ndim + 1)

	# Logistic_input = layer3.output
	...
	# params = layer0_q.params + layer0_a.params + layer2.params + layer3.params
	# params = layer0_q.params + layer0_a.params + layer2.params + layer3.params + [Embeddings]
	...
	cost = layer2.negative_log_likelihood(y) + reg_rate * ((layer0_q.params[0] ** 2).sum() + \
		(layer0_a.params[0] ** 2).sum() + (layer2.params[0] ** 2).sum())
	...
	for itera_i in range(itera_number):
		...
		df_weite = df_test.sort_values(by = 'qid')
		# print test_score
		...
		if test_mrr > best_mrr[0]:
			...
			stop_flag = 0
			df_weite.to_csv('test.result.csv', header=None,sep="\t",index=False, columns=['qid','aid',"question","answer","flag",'score'],quoting =3)
