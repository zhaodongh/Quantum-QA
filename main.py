...
from util import batch_pad, briany_test_file
from cnn_net import LogisticRegression, cnn_net, share_x, share_y, \
Density_Dot, Density
...
if __name__ == '__main__':
	...
	print("len(train_inter[1]",len(train_inter[1]))

	test_inter = Pickle.load(open(test_inter_fname, 'rb'))
	print("len(test_inter[1]",len(test_inter[1]))
	...
	input_shape = (embedding_ndim, embedding_ndim) #[50 * 50]
	...
	rng = np.random.RandomState(234)#youzh

	#implement our model by theano
	...
	#get the embedding from the pretrained embedding
	...
	params = layer0_q.params + layer0_a.params + layer1.params + layer2.params

	cost1 = layer2.negative_log_likelihood(y)
	cost = layer2.negative_log_likelihood(y) + reg_rate * ((layer0_q.params[0] ** 2).sum() + \
		(layer0_a.params[0] ** 2).sum() + (layer1.params[0] ** 2).sum() + (layer2.params[0] ** 2).sum())
	# cost = layer2.negative_log_likelihood(y) + reg_rate * ( \
	# 	(layer1.params[0] ** 2).sum() + (layer2.params[0] ** 2).sum())
	...
	train_model_hahah = theano.function([index],layer1.W, givens = given_train, on_unused_input='ignore', allow_input_downcast=True)
	train_model_hahah1 = theano.function([index], layer1.W.shape, givens = given_train, on_unused_input='ignore', allow_input_downcast=True)
	...
	train_model_loss1 = theano.function([index], cost1, givens = given_train, on_unused_input='ignore', allow_input_downcast=True)
	test_model_loss = theano.function([index], cost, givens = given_test, on_unused_input='ignore', allow_input_downcast=True)
	test_model_loss1 = theano.function([index], cost1, givens = given_test, on_unused_input='ignore', allow_input_downcast=True)
	...
	for itera_i in range(itera_number):
		...
		# 	print train_model_hahah1(i)
			...
			if count_i % 10 == 0:
				print count_i, 'train have been trained, loss is ', train_model_loss(i), train_model_loss1(i)
			count_i += 1
		...
		for i in range(test_batch_num):
			...
			if count_i % 10 == 0:
				print count_i, 'test have been tested, loss is ', test_model_loss(i), test_model_loss1(i)
			count_i += 1
		test_score = test_score[:test_sample_num]
		
		# df_test.to_csv(test_result_fname, header=None,sep="\t",index=False, names=['qid','aid',"question","answer","flag",'score'],quoting =3)
		# print test_score
